...
local ctrl_release_timer = nil
vim.keymap.set({ "n", "i" }, "<C-Tab>", function()
  M.select_next()

  -- 重置 timer，400ms 后检测 Ctrl 是否松开
  if ctrl_release_timer then
    ctrl_release_timer:close()
  end
  ctrl_release_timer = vim.defer_fn(function()
    M.confirm_switch()
    ctrl_release_timer = nil
  end, 400)
